...
Motivational Reason are practical reason for Agent to prefer certain course of action. Contrasted with normative reason that decide what Agent should do from impartial point of view. For example, Jane has high blood pressure a normative reason to not eat chocolate. But Jane has motivational reason to eat chocolate. Agent might not aware of normative reason but always aware of motivational reason. Also, normative always factual and motivational may deceive Agent. Like Jane's condition is a fact but the chocolate might not delicious (false belief). Ideally normative and motivational coincide, so the Agent are motivated by facts.

The relation between motivation and Agent belief is called ought-beliefs. These relation is the core of John Broome's enkratic rationality. "Rationality requires of you that, if you believe you ought to F, then you intend to F." Philosophy that reasoning is responsible for ought-beliefs led intention. Though rationality is not always fulfilled, which lead to irrationality. Which in turn lead to akrasia, if Agent can't fulfill enkratic requirement, if Agent think A must be done but doing B instead. Accidie is a similar phenomenon, when Agent believes something important need to be done but lacks motivation due to listlessness.

With many motivation theory, motivation control still very little understood. Various motivation training does exist, but many can be called pseudoscientific. To understand motivation control, lack of motivation need to be understood first.
